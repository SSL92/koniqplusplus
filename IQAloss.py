...
class IQALoss(torch.nn.Module):
    def __init__(self, loss_type='linearity', alpha=[1, 0], p=1, q=2, monotonicity_regularization=True, gamma=0.1, detach=False):
    ...
    def forward(self, y_pred, y):
        len_pred = y_pred.size()[1]
        y = y[0].view(-1, len_pred)
...
def norm_loss_with_normalization(y_pred, y, alpha=[1, 1], p=1, q=2, detach=False, exponent=True):
    ...
    if N > 1:  
        ...
        # print((alpha[0] * loss0 + alpha[1] * loss1) / (alpha[0] + alpha[1]))
        return (alpha[0] * loss0 + alpha[1] * loss1) / (alpha[0] + alpha[1])
    else:
        return F.l1_loss(y_pred, y_pred.detach())  # 0 for batch with single sample.
